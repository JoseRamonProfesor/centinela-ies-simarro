...
> **Nota — datos sinteticos de CAPTIA (pendiente):** estamos explorando con CAPTIA la posibilidad de que proporcionen datos sinteticos o anonimizados de una instalacion real, ya en formato InfluxDB con el schema canonico. Si esto se confirma, la capa plata de arranque ya estaria poblada y cada equipo podria construir directamente su capa oro. Os comunicaremos la respuesta de CAPTIA en cuanto la tengamos. Mientras tanto, planificad vuestro trabajo asumiendo que la capa plata la construis vosotros con ETL desde los datasets publicos.
...
```
G4-F (MLOps) -> convencion experimentos MLflow ----------> TODOS (semana 1-2)
G2-G (Calidad, si confirma cambio) -> reglas bronce -----> TODOS (semana 1-2)
